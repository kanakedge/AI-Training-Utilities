...
HOME = 'D:\en'
VAL_IMGS_DIR = HOME + r'\data\coco\images\val2017'
VAL_ANNOTATIONS = HOME + r'\data\coco\annotations\instances_val2017.json'
...
def generate_results(ssd, imgs_dir, jpgs, results_file):
    ...
    for jpg in progressbar(jpgs):
        ...
        predicted_boxes_count += len(boxes)
...
def get_accuracy():
    with open(VAL_ANNOTATIONS) as val_annotations:
        ground_truth_annotations = json.load(val_annotations)['annotations']
        ground_truth_boxes_count = len(ground_truth_annotations)

    print('Accuracy: {}'.format(predicted_boxes_count/ground_truth_boxes_count))
