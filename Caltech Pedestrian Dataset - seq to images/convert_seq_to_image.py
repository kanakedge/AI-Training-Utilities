...
python convert_seq_to_image.py -c <config_file_path>
OR
python convert_seq_to_image.py --config_path <config_file_path>
"""
...
class ConvertSEQtoImage:
    ...
    def read_seq(path):
        ...
        ifile = open(path, 'rb')
        params = ConvertSEQtoImage.read_header(ifile)
    ...
    def extract_images_video(data_path, save_path):
        ...
        imgs, ext = ConvertSEQtoImage.read_seq(data_path)
    ...
    def extract_annotations_video(data_path, save_path):
        ...
        data = ConvertSEQtoImage.read_vbb(data_path)
    ...
    def extract_files(data_path, save_path, sets):
        ...
        for j, set_name in enumerate(sets):
            ...
            for i, video in enumerate(fnames):
                ...
                ConvertSEQtoImage.extract_images_video(video_path, img_save_path)

                # extract annotations from .vbb file to .json
                ConvertSEQtoImage.extract_annotations_video(annot_path, annot_save_path)
    ...
    def extract_data(data_path, save_path, sets=None):
        ...
        ConvertSEQtoImage.extract_files(data_path, save_path, sets)
    ...
    def convert(config_path):
        ...
        assert any(data_path), 'Please provide a valid data path.'

        if not save_path:
            save_path = os.path.join(data_path, 'extracted_data')

        ConvertSEQtoImage.extract_data(data_path, save_path, sets)


if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('-c', '--config_path', default='', type=str, help='Config File Path')
    args = parser.parse_args()

    config_path = args.config_path

    assert any(config_path), 'Please insert a valid config file path using the -config_path input arg.'

    ConvertSEQtoImage.convert(config_path)
